...
export function HomeRightNow({ intraday, baselines }) {
  ...
  function compareToBaseline(key, val) {
    if ((val === null || val === undefined) || !baseline || !baseline[key]) return null;
  ...
  const items = [
    { label: 'Power (W)', value: latest.power_watts !== null && latest.power_watts !== undefined ? latest.power_watts : '\u2014', note: compareToBaseline('power_watts', latest.power_watts) },
    { label: 'Lights On', value: latest.lights_on !== null && latest.lights_on !== undefined ? latest.lights_on : '\u2014', note: compareToBaseline('lights_on', latest.lights_on) },
    { label: 'Devices Home', value: latest.devices_home !== null && latest.devices_home !== undefined ? latest.devices_home : '\u2014' },
    { label: 'Unavailable', value: latest.unavailable !== null && latest.unavailable !== undefined ? latest.unavailable : '\u2014', warning: (latest.unavailable || 0) > UNAVAILABLE_WARNING_THRESHOLD, note: (latest.unavailable || 0) > UNAVAILABLE_WARNING_THRESHOLD ? { text: 'High \u2014 check devices', style: 'color: var(--status-warning)' } : null },
  ];
